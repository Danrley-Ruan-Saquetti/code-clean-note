...
### Nomes Significativos
   * Use nomes que revelem seu propósito
      ...
      - Evite comentários para explicar o que tal variável/função/classe representa (Exemplo 1*)
...
### Evite informações falsas
   * Evite o uso de palavras que possa gerar ambiguidade
      ...
      - Evite palavras reservadas para nomeação (Exemplo 2*)
      - Evite nomes parecidos, fazendo distinções bem distintas entre elas para que não gere confusão na hora de decidir qual deve chamar (Exemplo 3*)
...
### Escreve nomes pronunciáveis
   * Escrever nomes que são difícies de pronunciar não é um hábito (Exemplo 4;5*)
...
### Escreve nomes possíveis de busca
   * Utilize nomes com um dígito apenas dentro de escopos locais e em métodos pequenos. "O tamanho de um nome deve ser proposcional ao tamanho do escopo" (Exemplo 6*)
...
### Nomes de Métodos
   ...
   * Quando nomes de construtores estiverem sobrecarregados, use métodos factory estáticos com nomes de descrevam os parâmetros (Exemplo 7*)
...
### Não dê uma de espertinho
   * Não use nomes que apenas o seu grupo interno saibam o significado, gírias, coloquialismos ou palavras de baixo calão (Exemplo 8*)
...
### Selecione uma palavra por conceito
   * Evite usar termos diferentes para o mesmo propósito como `pegar`, `obter` e `recuperar` ou `controlador` e `gerenciador` (Exemplo 9*)
...
### Adicione um contexto significativo
   * Nomear classes, funções e namespaces pode funcionar na maioria dos casos, mas, em últimos casos, quando não há um contexto definido, colocar um preixo pode ajudar (Exemplo 10*) Porém não adicione contexto exageradamente sem a necessidade
